...
local function refreshMyPlotSide()
    ...
    G_myPlotSide = side; G_myPlotName = plotName; return side
end
task.spawn(function()
    while true do task.wait(2); if G_tpAutoEnabled then refreshMyPlotSide() end end
end)
task.spawn(function()
    ...
    local function watchPlot(plot)
        pcall(function()
            local tl = plot:WaitForChild("PlotSign",5):WaitForChild("SurfaceGui",5)
                           :WaitForChild("Frame",5):WaitForChild("TextLabel",5)
...
local function isMyPlotByName(plotName)
    ...
    if sign then
        ...
        if yourBase and yourBase:IsA("BillboardGui") then
            ...
            plotCache[plotName]=result;plotCacheTime[plotName]=currentTime;return result
        end
    end
    ...
    if currentTime-promptCacheTime<PROMPT_CACHE_REFRESH and #cachedPrompts>0 then
        ...
        for _,data in ipairs(cachedPrompts) do
            if data.spawn then
                ...
                if dist<=Settings.StealRadius and dist<nd then
                    np=data.prompt;nd=dist;nn=data.name
                end
            end
        end
...
lines.append('''
-- ============================================================
-- ANTI-RAGDOLL  (new v1 implementation)
-- ============================================================
...
local function enableNoCameraCollision()
    ...
    noCamCollisionConn = RunService.RenderStepped:Connect(function()
        ...
        local dist    = toChar.Magnitude; if dist < 0.3 then return end
...
local function runDropBrainrot()
    ...
    dc = RunService.Heartbeat:Connect(function()
        ...
        if not r then
            ...
            if dropMobileSetter then dropMobileSetter(false) end; return
        end
...
local function runTPDown()
    ...
    task.spawn(function()
        pcall(function()
            ...
            local rp = RaycastParams.new()
            rp.FilterDescendantsInstances={c}; rp.FilterType=Enum.RaycastFilterType.Exclude
            ...
            if hit then
                ...
                h2.CFrame = CFrame.new(hit.Position.X,hit.Position.Y+(hum.HipHeight or 2)+(h2.Size.Y/2)+0.1,hit.Position.Z)
                h2.AssemblyLinearVelocity = Vector3.zero
            end
        end)
...
lines.append('''
-- ============================================================
-- VISUAL / OPTIMIZATION (original K7)
-- ============================================================
...
local function stopRemoveAccessories()
    ...
    if Connections.removeAccessories then
        Connections.removeAccessories:Disconnect(); Connections.removeAccessories=nil
    end
    removedAccessories={}
end
''')
...
lines.append('''
-- ============================================================
-- BRAINROT RETURN + AUTO TP (original K7)
-- ============================================================
local function doReturnTeleport(side)
    ...
    task.spawn(function()
        pcall(function()
            ...
            local hum=c:FindFirstChildOfClass("Humanoid"); if not root then return end
        ...
        if G_autoPlayAfterTP and G_myPlotSide then
            ...
            if fullAutoPlayLeftEnabled then
                stopFullAutoLeft(); fullAutoPlayLeftEnabled=false
                if fullAutoLeftSetter then fullAutoLeftSetter(false) end
            end
            if fullAutoPlayRightEnabled then
                stopFullAutoRight(); fullAutoPlayRightEnabled=false
...
lines.append('''
-- ============================================================
-- COUNTDOWN + FULL AUTO PLAY (original K7)
-- ============================================================
local function monitorCountdown(snd)
    ...
    conn=RunService.Heartbeat:Connect(function()
        ...
        if ct>COUNTDOWN_TARGET+2 then
            G_countdownActive=false; if conn then conn:Disconnect(); conn=nil end
        end
    end)
end
...
do
    local ex=workspace:FindFirstChild("Countdown")
    if ex and ex:IsA("Sound") then monitorCountdown(ex) end
end
...
local function getClosestPlayer()
    ...
    for _,p in pairs(Players:GetPlayers()) do
        if p~=LocalPlayer and p.Character then
            ...
            if tr then
                local d=(h2.Position-tr.Position).Magnitude
...
RunService.Heartbeat:Connect(function()
    ...
    if autoBatToggled and now-_batT>=1/30 then
        ...
        local c=LocalPlayer.Character; if c then
            local h2=c:FindFirstChild("HumanoidRootPart"); if h2 then
                local target=getClosestPlayer()
                if target and target.Character then
                    local tr=target.Character:FindFirstChild("HumanoidRootPart")
                    if tr then
                        local fp=tr.Position+tr.CFrame.LookVector*1.5
                        local dir=(fp-h2.Position).Unit
                        h2.AssemblyLinearVelocity=Vector3.new(dir.X*56.5,dir.Y*56.5,dir.Z*56.5)
                    end
                end
            end
        end
...
local function saveConfig()
    local cfg={
        ...
        carryMode=speedToggled, laggerMode=laggerToggled,
        ...
        fastestSteal=fastestStealEnabled, autoSwing=autoSwingEnabled,
        noCamCollision=noCamCollisionEnabled, ultraMode=ultraModeEnabled,
        tpAutoEnabled=G_tpAutoEnabled, autoPlayAfterTP=G_autoPlayAfterTP,
        harderHitAnim=harderHitAnimEnabled, medusaCounter=medusaCounterEnabled,
        brainrotReturnLeft=brainrotReturnLeftEnabled,
        brainrotReturnRight=brainrotReturnRightEnabled,
    }
    if writefile then
        pcall(function() writefile("K7HubConfig.json",HttpService:JSONEncode(cfg)) end)
    end
end
    ...
    if cfg.carryMode ~=nil then
        ...
        task.defer(function()
            if speedSwBg and speedSwCircle then
                speedSwBg.BackgroundColor3 = speedToggled and Color3.fromRGB(60,210,255) or Color3.fromRGB(18,22,45)
                speedSwCircle.Position = speedToggled and UDim2.fromOffset(23,3) or UDim2.fromOffset(3,3)
            end
        end)
    end
    if cfg.laggerMode ~=nil then
        ...
        task.defer(function()
            if laggerSwBg and laggerSwCircle then
                laggerSwBg.BackgroundColor3 = laggerToggled and Color3.fromRGB(255,80,60) or Color3.fromRGB(18,22,45)
                laggerSwCircle.Position = laggerToggled and UDim2.fromOffset(23,3) or UDim2.fromOffset(3,3)
            end
...
-- PALETTE
local SB   = Color3.fromRGB(8,10,18)      -- panel bg
local SC   = Color3.fromRGB(12,15,28)     -- card
local SCH  = Color3.fromRGB(16,20,38)     -- card hover
local SH   = Color3.fromRGB(6,8,14)       -- header
local SA   = Color3.fromRGB(60,140,255)   -- accent blue
local SAD  = Color3.fromRGB(20,50,120)    -- accent dark
local SS   = Color3.fromRGB(40,80,180)    -- stroke
local SSD  = Color3.fromRGB(20,35,80)     -- stroke dim
local ST   = Color3.fromRGB(220,235,255)  -- text
local SM   = Color3.fromRGB(100,120,170)  -- muted
local SDM  = Color3.fromRGB(50,65,110)    -- dim
local SOFF = Color3.fromRGB(18,22,45)     -- toggle off
local SRED = Color3.fromRGB(220,30,30)    -- red
local SW   = Color3.fromRGB(255,255,255)  -- white
local SMINT= Color3.fromRGB(60,255,160)
local SAMB = Color3.fromRGB(255,190,60)
local SCYAN= Color3.fromRGB(60,210,255)
local SPINK= Color3.fromRGB(255,80,180)
...
local oldGui = LocalPlayer.PlayerGui:FindFirstChild("SweetyK7Hub")
if oldGui then oldGui:Destroy() end

gui = Instance.new("ScreenGui")
gui.Name="SweetyK7Hub"; gui.ResetOnSpawn=false
gui.ZIndexBehavior=Enum.ZIndexBehavior.Sibling
gui.DisplayOrder=99; gui.IgnoreGuiInset=true
gui.Parent=LocalPlayer.PlayerGui

-- ── HUD (FPS + Ping pills) ──────────────────────────────────
local HUD = Instance.new("Frame")
HUD.Size=UDim2.new(0,200,0,28); HUD.Position=UDim2.new(0.5,-100,0,8)
HUD.BackgroundColor3=Color3.fromRGB(4,6,14); HUD.BackgroundTransparency=0.1
HUD.BorderSizePixel=0; HUD.ZIndex=20; HUD.Parent=gui
...
local function makePill(xPos, tag)
    local pill=Instance.new("Frame"); pill.Size=UDim2.new(0,88,0,20)
    pill.Position=UDim2.new(xPos,xPos==0 and 6 or -94,0.5,-10)
    pill.BackgroundColor3=SAD; pill.BorderSizePixel=0; pill.Parent=HUD
    Instance.new("UICorner",pill).CornerRadius=UDim.new(0,6)
    local tagL=Instance.new("TextLabel",pill); tagL.Size=UDim2.new(0,28,1,0)
    tagL.BackgroundTransparency=1; tagL.Text=tag; tagL.TextSize=9
    tagL.Font=Enum.Font.GothamBold; tagL.TextColor3=SA
    local valL=Instance.new("TextLabel",pill); valL.Size=UDim2.new(1,-28,1,0)
    valL.Position=UDim2.new(0,28,0,0); valL.BackgroundTransparency=1; valL.Text="--"
    valL.Font=Enum.Font.GothamBold; valL.TextSize=11; valL.TextColor3=ST
    valL.TextXAlignment=Enum.TextXAlignment.Left
    return valL
end
local fpsLbl  = makePill(0,"FPS")
local pingLbl = makePill(1,"MS")
local divH=Instance.new("Frame"); divH.Size=UDim2.new(0,1,0,16)
divH.Position=UDim2.new(0.5,0,0.5,-8); divH.BackgroundColor3=SSD; divH.BorderSizePixel=0; divH.Parent=HUD

local fpsAcc=0; local fpsT=0
RunService.Heartbeat:Connect(function(dt)
    fpsT=fpsT+dt; fpsAcc=fpsAcc+1
    if fpsT>=0.5 then
        local f=math.round(fpsAcc/fpsT)
        fpsLbl.Text=f.." FPS"
        fpsLbl.TextColor3=f>=50 and SMINT or (f>=30 and SAMB or SRED)
        fpsT=0; fpsAcc=0
    end
    pcall(function()
        local ping=math.round(Stats.Network.ServerStatsItem["Data Ping"]:GetValue())
        pingLbl.Text=ping.." ms"
        pingLbl.TextColor3=ping<=80 and SMINT or (ping<=150 and SAMB or SRED)
    end)
end)

-- ── LOADING SCREEN  (void blue, clean card, NO circle/glow/lines) ──
local Loader=Instance.new("Frame"); Loader.Size=UDim2.new(1,0,1,0)
Loader.BackgroundColor3=Color3.fromRGB(3,5,12); Loader.BorderSizePixel=0
Loader.ZIndex=50; Loader.Parent=gui

local LoadCard=Instance.new("Frame"); LoadCard.Size=UDim2.new(0,400,0,220)
LoadCard.Position=UDim2.new(0.5,-200,0.5,-110)
LoadCard.BackgroundColor3=Color3.fromRGB(7,10,22); LoadCard.BorderSizePixel=0
LoadCard.ZIndex=51; LoadCard.Parent=Loader
Instance.new("UICorner",LoadCard).CornerRadius=UDim.new(0,18)
local cSt=Instance.new("UIStroke",LoadCard); cSt.Color=SA; cSt.Thickness=1.5; cSt.Transparency=0.3

-- blue gradient top stripe
local topStripe=Instance.new("Frame"); topStripe.Size=UDim2.new(1,0,0,3)
topStripe.BackgroundColor3=SA; topStripe.BorderSizePixel=0; topStripe.ZIndex=53; topStripe.Parent=LoadCard
Instance.new("UICorner",topStripe).CornerRadius=UDim.new(0,18)
local tsg=Instance.new("UIGradient",topStripe)
tsg.Color=ColorSequence.new({ColorSequenceKeypoint.new(0,Color3.fromRGB(30,80,200)),ColorSequenceKeypoint.new(0.5,SA),ColorSequenceKeypoint.new(1,Color3.fromRGB(120,200,255))})

-- icon circle
local iconBg=Instance.new("Frame"); iconBg.Size=UDim2.new(0,56,0,56)
iconBg.Position=UDim2.new(0.5,-28,0,18); iconBg.BackgroundColor3=SAD
iconBg.BorderSizePixel=0; iconBg.ZIndex=53; iconBg.Parent=LoadCard
...
local iconLbl=Instance.new("TextLabel",iconBg); iconLbl.Size=UDim2.new(1,0,1,0)
iconLbl.BackgroundTransparency=1; iconLbl.Text="K7"; iconLbl.TextSize=22
iconLbl.Font=Enum.Font.GothamBlack; iconLbl.TextColor3=SA; iconLbl.ZIndex=54

-- title
local lbTitle=Instance.new("TextLabel",LoadCard); lbTitle.Size=UDim2.new(1,-20,0,36)
lbTitle.Position=UDim2.new(0,10,0,82); lbTitle.BackgroundTransparency=1
lbTitle.Text="K7 HUB"; lbTitle.TextSize=28; lbTitle.Font=Enum.Font.GothamBlack
lbTitle.TextColor3=ST; lbTitle.ZIndex=52

-- divider line
local lbDiv=Instance.new("Frame",LoadCard); lbDiv.Size=UDim2.new(0,180,0,2)
lbDiv.Position=UDim2.new(0.5,-90,0,122); lbDiv.BackgroundColor3=SA
lbDiv.BorderSizePixel=0; lbDiv.ZIndex=53
Instance.new("UICorner",lbDiv).CornerRadius=UDim.new(1,0)
local divGrad=Instance.new("UIGradient",lbDiv)
divGrad.Color=ColorSequence.new({ColorSequenceKeypoint.new(0,Color3.fromRGB(20,60,180)),ColorSequenceKeypoint.new(1,SA)})

-- subtitle
local lbSub=Instance.new("TextLabel",LoadCard); lbSub.Size=UDim2.new(1,-20,0,16)
lbSub.Position=UDim2.new(0,10,0,130); lbSub.BackgroundTransparency=1
lbSub.Text="Steal a Brainrot  |  discord.gg/k7hub"
lbSub.TextSize=10; lbSub.Font=Enum.Font.GothamMedium
lbSub.TextColor3=Color3.fromRGB(80,110,180); lbSub.ZIndex=52

-- progress bar
local barTrack=Instance.new("Frame",LoadCard); barTrack.Size=UDim2.new(0,320,0,5)
barTrack.Position=UDim2.new(0.5,-160,0,162); barTrack.BackgroundColor3=Color3.fromRGB(12,18,40)
barTrack.BorderSizePixel=0; barTrack.ZIndex=52
Instance.new("UICorner",barTrack).CornerRadius=UDim.new(1,0)
local barFill=Instance.new("Frame",barTrack); barFill.Size=UDim2.new(0,0,1,0)
barFill.BackgroundColor3=SA; barFill.BorderSizePixel=0; barFill.ZIndex=53
Instance.new("UICorner",barFill).CornerRadius=UDim.new(1,0)
local bfg=Instance.new("UIGradient",barFill)
bfg.Color=ColorSequence.new({ColorSequenceKeypoint.new(0,Color3.fromRGB(30,80,200)),ColorSequenceKeypoint.new(0.6,SA),ColorSequenceKeypoint.new(1,Color3.fromRGB(120,200,255))})

local loadSt=Instance.new("TextLabel",LoadCard); loadSt.Size=UDim2.new(1,-20,0,14)
loadSt.Position=UDim2.new(0,10,0,178); loadSt.BackgroundTransparency=1
loadSt.TextSize=10; loadSt.Font=Enum.Font.GothamMedium
loadSt.TextColor3=Color3.fromRGB(50,80,140); loadSt.ZIndex=52

local loadMsgs={"Initializing features...","Loading keybinds...","Preparing auto steal...","Anti-ragdoll ready...","Almost done!"}
local loadMsgI=1
task.spawn(function()
    while Loader and Loader.Parent do
        task.wait(0.4); loadMsgI=loadMsgI%#loadMsgs+1
        pcall(function() loadSt.Text=loadMsgs[loadMsgI] end)
    end
end)
...
-- ── MAIN PANEL shadow + frame ────────────────────────────────
local Shadow=Instance.new("Frame"); Shadow.Size=UDim2.new(0,292,0,10)
Shadow.Position=UDim2.new(0.5,-146,0.5,-240)
Shadow.BackgroundColor3=SA; Shadow.BackgroundTransparency=0.82
Shadow.BorderSizePixel=0; Shadow.AutomaticSize=Enum.AutomaticSize.Y; Shadow.Parent=gui
Instance.new("UICorner",Shadow).CornerRadius=UDim.new(0,20)

main=Instance.new("Frame"); main.Size=UDim2.new(0,284,0,0)
main.Position=UDim2.new(0,4,0,4); main.BackgroundColor3=SB
main.BorderSizePixel=0; main.ClipsDescendants=true
main.AutomaticSize=Enum.AutomaticSize.Y; main.Parent=Shadow
Instance.new("UICorner",main).CornerRadius=UDim.new(0,16)
local PStroke=Instance.new("UIStroke",main); PStroke.Color=SS; PStroke.Thickness=1.5; PStroke.Transparency=0.25

-- ── HEADER ───────────────────────────────────────────────────
local Header=Instance.new("Frame",main); Header.Size=UDim2.new(1,0,0,54)
Header.BackgroundColor3=SH; Header.BorderSizePixel=0
Instance.new("UICorner",Header).CornerRadius=UDim.new(0,16)
local hFix=Instance.new("Frame",Header); hFix.Size=UDim2.new(1,0,0,16)
hFix.Position=UDim2.new(0,0,1,-16); hFix.BackgroundColor3=SH; hFix.BorderSizePixel=0

local hIco=Instance.new("TextLabel",Header); hIco.Size=UDim2.new(0,32,0,32)
hIco.Position=UDim2.new(0,12,0.5,-16); hIco.BackgroundColor3=SAD; hIco.BorderSizePixel=0
hIco.Text="K7"; hIco.TextSize=13; hIco.Font=Enum.Font.GothamBlack; hIco.TextColor3=SA
Instance.new("UICorner",hIco).CornerRadius=UDim.new(0,8)

local hTL=Instance.new("TextLabel",Header); hTL.Size=UDim2.new(0,160,0,20)
hTL.Position=UDim2.new(0,52,0,8); hTL.BackgroundTransparency=1; hTL.Text="K7 HUB"
hTL.TextSize=15; hTL.Font=Enum.Font.GothamBlack; hTL.TextColor3=SW
hTL.TextXAlignment=Enum.TextXAlignment.Left

local hSL=Instance.new("TextLabel",Header); hSL.Size=UDim2.new(0,200,0,13)
hSL.Position=UDim2.new(0,52,0,29); hSL.BackgroundTransparency=1; hSL.Text="discord.gg/k7hub"
hSL.TextSize=9; hSL.Font=Enum.Font.GothamMedium; hSL.TextColor3=SM
hSL.TextXAlignment=Enum.TextXAlignment.Left

local hChev=Instance.new("TextButton",Header); hChev.Size=UDim2.new(0,28,0,28)
hChev.Position=UDim2.new(1,-34,0.5,-14); hChev.BackgroundColor3=SAD; hChev.BorderSizePixel=0
hChev.Text="v"; hChev.TextSize=11; hChev.Font=Enum.Font.GothamBold; hChev.TextColor3=SM
hChev.AutoButtonColor=false; hChev.ZIndex=10
Instance.new("UICorner",hChev).CornerRadius=UDim.new(0,6)

local hLine=Instance.new("Frame",Header); hLine.Size=UDim2.new(0.9,0,0,1)
hLine.Position=UDim2.new(0.05,0,1,-1); hLine.BackgroundColor3=SSD; hLine.BorderSizePixel=0
Instance.new("UICorner",hLine).CornerRadius=UDim.new(1,0)

-- minimize
local isMin=false; local TabBar2, ContentWrap2
local function setMin(m)
    isMin=m; hChev.Text=m and "^" or "v"
    if TabBar2 then TabBar2.Visible=not m end
    if ContentWrap2 then ContentWrap2.Visible=not m end
    if m then
...
do
    ...
    UserInputService.InputChanged:Connect(function(inp)
        if drag and inp.UserInputType==Enum.UserInputType.MouseMovement then
            ...
            Shadow.Position=UDim2.new(ss.X.Scale,ss.X.Offset+d.X,ss.Y.Scale,ss.Y.Offset+d.Y)
        end
    end)
end
...
reopenBtn.Size=UDim2.new(0,45,0,45); reopenBtn.Position=UDim2.new(0,10,0,50)
reopenBtn.BackgroundColor3=SA; reopenBtn.BorderSizePixel=0; reopenBtn.Text="K7"
reopenBtn.TextColor3=SW; reopenBtn.Font=Enum.Font.GothamBlack; reopenBtn.TextSize=14
reopenBtn.ZIndex=20; reopenBtn.Visible=false
Instance.new("UICorner",reopenBtn).CornerRadius=UDim.new(0,10)

-- ── TAB BAR ──────────────────────────────────────────────────
TabBar2=Instance.new("Frame",main); TabBar2.Size=UDim2.new(1,0,0,34)
TabBar2.Position=UDim2.new(0,0,0,54); TabBar2.BackgroundColor3=SH; TabBar2.BorderSizePixel=0
local tbLine=Instance.new("Frame",TabBar2); tbLine.Size=UDim2.new(1,0,0,1)
tbLine.Position=UDim2.new(0,0,1,-1); tbLine.BackgroundColor3=SSD; tbLine.BorderSizePixel=0

local tabPages={}
local curPage=nil
local function mkTabBtn(lbl,xScale,wScale)
    local b=Instance.new("TextButton",TabBar2)
    b.Size=UDim2.new(wScale,0,1,0); b.Position=UDim2.new(xScale,0,0,0)
    b.BackgroundTransparency=1; b.Text=lbl; b.Font=Enum.Font.GothamBold
    b.TextSize=8; b.TextColor3=SM; b.AutoButtonColor=false
    local u=Instance.new("Frame",b); u.Size=UDim2.new(0.7,0,0,2)
    u.Position=UDim2.new(0.15,0,1,-2); u.BackgroundColor3=SA
    u.BorderSizePixel=0; u.BackgroundTransparency=1
...
allTabs={{tCB,tCU},{tMB,tMU},{tVB,tVU},{tAB,tAU}}

ContentWrap2=Instance.new("ScrollingFrame",main)
ContentWrap2.Size=UDim2.new(1,0,0,380); ContentWrap2.Position=UDim2.new(0,0,0,88)
ContentWrap2.BackgroundTransparency=1; ContentWrap2.BorderSizePixel=0
ContentWrap2.ScrollBarThickness=3; ContentWrap2.ScrollBarImageColor3=SS
ContentWrap2.CanvasSize=UDim2.new(0,0,0,0); ContentWrap2.AutomaticCanvasSize=Enum.AutomaticSize.Y
ContentWrap2.ElasticBehavior=Enum.ElasticBehavior.Never

local function mkPage()
    local p=Instance.new("Frame",ContentWrap2)
    p.Size=UDim2.new(1,0,0,0); p.BackgroundTransparency=1
    p.AutomaticSize=Enum.AutomaticSize.Y; p.Visible=false
    local ll=Instance.new("UIListLayout",p); ll.Padding=UDim.new(0,6)
...
local pgCombat  = mkPage()
local pgMove    = mkPage()
local pgVisual  = mkPage()
local pgAuto    = mkPage()

local function switchTab(page,btn,ul)
    if curPage then curPage.Visible=false end
    for _,t in ipairs(allTabs) do
        tw(t[1],TIF,{TextColor3=SM}); tw(t[2],TIF,{BackgroundTransparency=1})
    end
    page.Visible=true; curPage=page; ContentWrap2.CanvasPosition=Vector2.new(0,0)
...
reopenBtn.MouseButton1Click:Connect(function()
    Shadow.Visible=true; reopenBtn.Visible=false
end)

-- ── SECTION HEADER ────────────────────────────────────────────
local function mkSec(parent,lo,lbl)
    local r=Instance.new("Frame",parent)
    r.Size=UDim2.new(1,-20,0,20); r.BackgroundTransparency=1; r.LayoutOrder=lo
    local l1=Instance.new("Frame",r); l1.Size=UDim2.new(0.26,0,0,1)
    l1.Position=UDim2.new(0,0,0.5,0); l1.BackgroundColor3=SSD; l1.BorderSizePixel=0
    Instance.new("UICorner",l1).CornerRadius=UDim.new(1,0)
    local lb=Instance.new("TextLabel",r); lb.Size=UDim2.new(0.48,0,1,0)
    lb.Position=UDim2.new(0.26,0,0,0); lb.BackgroundTransparency=1; lb.Text=lbl
    lb.Font=Enum.Font.GothamBold; lb.TextSize=9; lb.TextColor3=SA
    local l2=Instance.new("Frame",r); l2.Size=UDim2.new(0.26,0,0,1)
    l2.Position=UDim2.new(0.74,0,0.5,0); l2.BackgroundColor3=SSD; l2.BorderSizePixel=0
...
local function mkToggle(parent,lo,label,keybindName,color,defaultOn,onToggle)
    ...
    local card=Instance.new("Frame",parent); card.Size=UDim2.new(1,-20,0,60)
    card.BackgroundColor3=SC; card.BorderSizePixel=0; card.LayoutOrder=lo
    Instance.new("UICorner",card).CornerRadius=UDim.new(0,12)
    local cSt2=Instance.new("UIStroke",card); cSt2.Color=SSD; cSt2.Thickness=1.2; cSt2.Transparency=0.4
    local bar=Instance.new("Frame",card); bar.Size=UDim2.new(0,3,0,32)
    bar.Position=UDim2.new(0,0,0.5,-16); bar.BackgroundColor3=safeColor; bar.BorderSizePixel=0
    Instance.new("UICorner",bar).CornerRadius=UDim.new(1,0)

    local xOff=10; local badge,badgeTxt
    if keybindName and keybindName~="" and Keybinds[keybindName] then
        badge=Instance.new("Frame",card); badge.Size=UDim2.new(0,28,0,20)
        badge.Position=UDim2.new(0,10,0.5,-10); badge.BackgroundColor3=SA
        badge.BackgroundTransparency=0.3; badge.BorderSizePixel=0; badge.ZIndex=5
        Instance.new("UICorner",badge).CornerRadius=UDim.new(0,6)
        badgeTxt=Instance.new("TextLabel",badge); badgeTxt.Size=UDim2.new(1,0,1,0)
        badgeTxt.BackgroundTransparency=1; badgeTxt.Text=Keybinds[keybindName].Name
        badgeTxt.TextColor3=SW; badgeTxt.Font=Enum.Font.GothamBold
        badgeTxt.TextSize=10; badgeTxt.ZIndex=6
        xOff=45
    else
        local icoBg=Instance.new("Frame",card); icoBg.Size=UDim2.new(0,36,0,36)
        icoBg.Position=UDim2.new(0,10,0.5,-18); icoBg.BackgroundColor3=SAD
        icoBg.BorderSizePixel=0; icoBg.ZIndex=5
        Instance.new("UICorner",icoBg).CornerRadius=UDim.new(0,9)
        Instance.new("UIStroke",icoBg).Color=safeColor
        xOff=54
    end

    local nameLbl=Instance.new("TextLabel",card); nameLbl.Size=UDim2.new(1,-148,0,20)
    nameLbl.Position=UDim2.new(0,xOff,0,10); nameLbl.BackgroundTransparency=1
    nameLbl.Text=label; nameLbl.Font=Enum.Font.GothamBold; nameLbl.TextSize=12
    nameLbl.TextColor3=ST; nameLbl.TextXAlignment=Enum.TextXAlignment.Left

    local hintLbl=Instance.new("TextLabel",card); hintLbl.Size=UDim2.new(1,-148,0,13)
    hintLbl.Position=UDim2.new(0,xOff,0,30); hintLbl.BackgroundTransparency=1
    hintLbl.Text=keybindName and (keybindName~="" and "Hold to rebind" or "") or ""
    hintLbl.Font=Enum.Font.Gotham; hintLbl.TextSize=9
    hintLbl.TextColor3=defaultOn and safeColor or SDM; hintLbl.TextXAlignment=Enum.TextXAlignment.Left

    local track=Instance.new("Frame",card); track.Size=UDim2.new(0,44,0,24)
    track.Position=UDim2.new(1,-52,0.5,-12)
    track.BackgroundColor3=defaultOn and safeColor or SOFF; track.BorderSizePixel=0
    ...
    knob.Position=defaultOn and UDim2.fromOffset(23,3) or UDim2.fromOffset(3,3)
    knob.BackgroundColor3=defaultOn and SW or SM; knob.BorderSizePixel=0
    Instance.new("UICorner",knob).CornerRadius=UDim.new(1,0)

    local btn=Instance.new("TextButton",card); btn.Size=UDim2.new(1,0,1,0)
    ...
    local function setVis(on)
        ...
        trkSt.Color=on and safeColor or SSD
        hintLbl.TextColor3=on and safeColor or SDM
    ...
    if badge and badgeTxt and keybindName and keybindName~="" then
        local function startKBChange()
            ...
            local conn; conn=UserInputService.InputBegan:Connect(function(inp,_)
                ...
                if not (inp.UserInputType==Enum.UserInputType.Keyboard
                     or inp.UserInputType==Enum.UserInputType.Gamepad1) then return end
                local kc=inp.KeyCode; if kc==Enum.KeyCode.Unknown then return end
                if kc==Enum.KeyCode.Escape then
                    badgeTxt.Text=Keybinds[keybindName].Name
                    tw(badge,TIF,{BackgroundColor3=SA,BackgroundTransparency=0.3})
                    waitingForKeySW=nil; conn:Disconnect(); return
                end
                Keybinds[keybindName]=kc; badgeTxt.Text=kc.Name
                tw(badge,TIF,{BackgroundColor3=SA,BackgroundTransparency=0.3})
                waitingForKeySW=nil; conn:Disconnect()
            end)
        end
        btn.MouseButton1Down:Connect(function()
            isHolding=false; holdStart=tick()
            task.delay(0.6,function()
                if holdStart>0 and (tick()-holdStart)>=0.6 then isHolding=true; startKBChange() end
            end)
        end)
        btn.MouseButton1Up:Connect(function()
            if (tick()-holdStart)<0.6 then holdStart=0 end
        end)
        btn.MouseButton2Click:Connect(function() startKBChange() end)
    end

    btn.MouseButton1Click:Connect(function()
        if not isHolding then isOn=not isOn; setVis(isOn); if onToggle then onToggle(isOn) end end
...
local function mkInput(parent,lo,lbl,defaultTxt,onDone)
    ...
    container.Size=UDim2.new(1,-20,0,48); container.BackgroundColor3=SC
    container.BorderSizePixel=0; container.LayoutOrder=lo
    Instance.new("UICorner",container).CornerRadius=UDim.new(0,12)
    local cSt2=Instance.new("UIStroke",container); cSt2.Color=SSD; cSt2.Thickness=1.2; cSt2.Transparency=0.4
    local bar2=Instance.new("Frame",container); bar2.Size=UDim2.new(0,3,0,28)
    bar2.Position=UDim2.new(0,0,0.5,-14); bar2.BackgroundColor3=SA; bar2.BorderSizePixel=0
    Instance.new("UICorner",bar2).CornerRadius=UDim.new(1,0)
    local lblEl=Instance.new("TextLabel",container); lblEl.Size=UDim2.new(0.5,0,1,0)
    lblEl.Position=UDim2.new(0,16,0,0); lblEl.BackgroundTransparency=1; lblEl.Text=lbl
    lblEl.TextColor3=ST; lblEl.Font=Enum.Font.GothamBold; lblEl.TextSize=12; lblEl.TextXAlignment=Enum.TextXAlignment.Left
    local box=Instance.new("TextBox",container); box.Size=UDim2.new(0,80,0,28)
    box.Position=UDim2.new(1,-90,0.5,-14); box.BackgroundColor3=SH; box.BorderSizePixel=0
    box.Text=defaultTxt; box.TextColor3=ST; box.Font=Enum.Font.GothamBold; box.TextSize=11
...
-- ── PROGRESS BAR (steal progress) ────────────────────────────
local progressBar=Instance.new("Frame",gui)
progressBar.Size=UDim2.new(0,500,0,58); progressBar.Position=UDim2.new(0.5,-250,1,-72)
progressBar.BackgroundColor3=SB; progressBar.BorderSizePixel=0; progressBar.Active=true; progressBar.ZIndex=50
...
ProgressLabel=Instance.new("TextLabel",progressBar); ProgressLabel.Size=UDim2.new(0.35,0,0,18)
ProgressLabel.Position=UDim2.new(0,10,0,8); ProgressLabel.BackgroundTransparency=1
ProgressLabel.Text="READY"; ProgressLabel.TextColor3=SA; ProgressLabel.Font=Enum.Font.GothamBlack
ProgressLabel.TextSize=12; ProgressLabel.TextXAlignment=Enum.TextXAlignment.Left; ProgressLabel.ZIndex=51

ProgressPercentLabel=Instance.new("TextLabel",progressBar); ProgressPercentLabel.Size=UDim2.new(1,-10,0,18)
ProgressPercentLabel.Position=UDim2.new(0,0,0,8); ProgressPercentLabel.BackgroundTransparency=1
ProgressPercentLabel.Text=""; ProgressPercentLabel.TextColor3=SW; ProgressPercentLabel.Font=Enum.Font.GothamBlack
ProgressPercentLabel.TextSize=13; ProgressPercentLabel.TextXAlignment=Enum.TextXAlignment.Right; ProgressPercentLabel.ZIndex=51

local radLbl=Instance.new("TextLabel",progressBar); radLbl.Size=UDim2.new(0,52,0,14)
radLbl.Position=UDim2.new(0,10,0,30); radLbl.BackgroundTransparency=1; radLbl.Text="Radius:"
radLbl.TextColor3=SM; radLbl.Font=Enum.Font.Gotham; radLbl.TextSize=10; radLbl.TextXAlignment=Enum.TextXAlignment.Left; radLbl.ZIndex=51

RadiusInput=Instance.new("TextBox",progressBar); RadiusInput.Size=UDim2.new(0,52,0,20)
RadiusInput.Position=UDim2.new(0,64,0,28); RadiusInput.BackgroundColor3=SC; RadiusInput.BorderSizePixel=0
RadiusInput.Text=tostring(Settings.StealRadius); RadiusInput.TextColor3=ST
RadiusInput.Font=Enum.Font.GothamBold; RadiusInput.TextSize=10; RadiusInput.ZIndex=51
Instance.new("UICorner",RadiusInput).CornerRadius=UDim.new(0,5)
Instance.new("UIStroke",RadiusInput).Color=SS
RadiusInput.FocusLost:Connect(function()
    local n=tonumber(RadiusInput.Text)
    if n then Settings.StealRadius=math.clamp(math.floor(n),1,500); Values.STEAL_RADIUS=Settings.StealRadius; RadiusInput.Text=tostring(Settings.StealRadius); cachedPrompts={}; promptCacheTime=0 end
end)

local pTrack=Instance.new("Frame",progressBar); pTrack.Size=UDim2.new(1,-240,0,16)
pTrack.Position=UDim2.new(0,128,0,32); pTrack.BackgroundColor3=SOFF; pTrack.BorderSizePixel=0; pTrack.ZIndex=50
Instance.new("UICorner",pTrack).CornerRadius=UDim.new(0,5)
ProgressBarFill=Instance.new("Frame",pTrack); ProgressBarFill.Size=UDim2.new(0,0,1,0)
ProgressBarFill.BackgroundColor3=SA; ProgressBarFill.BorderSizePixel=0; ProgressBarFill.ZIndex=51
Instance.new("UICorner",ProgressBarFill).CornerRadius=UDim.new(0,5)
local bfg2=Instance.new("UIGradient",ProgressBarFill)
bfg2.Color=ColorSequence.new({ColorSequenceKeypoint.new(0,Color3.fromRGB(30,80,200)),ColorSequenceKeypoint.new(1,Color3.fromRGB(120,200,255))})

local durLbl=Instance.new("TextLabel",progressBar); durLbl.Size=UDim2.new(0,52,0,14)
durLbl.Position=UDim2.new(1,-128,0,30); durLbl.BackgroundTransparency=1; durLbl.Text="Dur:"
durLbl.TextColor3=SM; durLbl.Font=Enum.Font.Gotham; durLbl.TextSize=10; durLbl.TextXAlignment=Enum.TextXAlignment.Right; durLbl.ZIndex=51
DurationInput=Instance.new("TextBox",progressBar); DurationInput.Size=UDim2.new(0,64,0,20)
DurationInput.Position=UDim2.new(1,-68,0,28); DurationInput.BackgroundColor3=SC; DurationInput.BorderSizePixel=0
DurationInput.Text=tostring(Settings.StealDuration); DurationInput.TextColor3=ST
DurationInput.Font=Enum.Font.GothamBold; DurationInput.TextSize=10; DurationInput.ZIndex=51
Instance.new("UICorner",DurationInput).CornerRadius=UDim.new(0,5)
Instance.new("UIStroke",DurationInput).Color=SS
DurationInput.FocusLost:Connect(function()
    local n=tonumber(DurationInput.Text)
    if n then Settings.StealDuration=math.clamp(n,0.05,5); Values.STEAL_DURATION=Settings.StealDuration; DurationInput.Text=string.format("%.2f",Settings.StealDuration) end
end)
...
local arSetVis,_,_ = mkToggle(pgCombat,O(),"Anti Ragdoll","",Color3.fromRGB(255,80,180),Enabled.AntiRagdoll,function(s)
    Enabled.AntiRagdoll=s
    if s then startAntiRagdoll() else stopAntiRagdoll() end
end)
VisualSetters.AntiRagdoll = arSetVis

local brSwBgL2,brSwCircleL2,brSwBgR2,brSwCircleR2
local brLVis,_brBgL,_brCirL = mkToggle(pgCombat,O(),"Brainrot Return L","",SMINT,false,function(on)
    if on and brainrotReturnRightEnabled then
        ...
        if brSwBgR2 then tw(brSwBgR2,TIF,{BackgroundColor3=SOFF}) end
        if brSwCircleR2 then tw(brSwCircleR2,TIF,{Position=UDim2.fromOffset(3,3)}) end
    end
    brainrotReturnLeftEnabled=on
end)
brSwBgL2=_brBgL; brSwCircleL2=_brCirL
...
local brRVis,_brBgR,_brCirR = mkToggle(pgCombat,O(),"Brainrot Return R","",SAMB,false,function(on)
    if on and brainrotReturnLeftEnabled then
        ...
        if brSwBgL2 then tw(brSwBgL2,TIF,{BackgroundColor3=SOFF}) end
        if brSwCircleL2 then tw(brSwCircleL2,TIF,{Position=UDim2.fromOffset(3,3)}) end
    end
    brainrotReturnRightEnabled=on
end)
brSwBgR2=_brBgR; brSwCircleR2=_brCirR
...
local carrySetVis,_cBg,_cCir = mkToggle(pgCombat,O(),"Carry Mode","SpeedToggle",SCYAN,speedToggled,function(on)
    if on and laggerToggled then
        laggerToggled=false
        if laggerSwBg and laggerSwCircle then tw(laggerSwBg,TIF,{BackgroundColor3=SOFF}); tw(laggerSwCircle,TIF,{Position=UDim2.fromOffset(3,3)}) end
...
local lagSetVis,_lBg,_lCir = mkToggle(pgCombat,O(),"Lagger Mode","LaggerToggle",Color3.fromRGB(255,80,60),laggerToggled,function(on)
    if on and speedToggled then
        speedToggled=false
        if speedSwBg and speedSwCircle then tw(speedSwBg,TIF,{BackgroundColor3=SOFF}); tw(speedSwCircle,TIF,{Position=UDim2.fromOffset(3,3)}) end
...
local batSetVis,_bBg,_bCir = mkToggle(pgCombat,O(),"Auto Bat","AutoBat",SA,autoBatToggled,function(on)
...
local hhSetVis,_,_ = mkToggle(pgCombat,O(),"Harder Hit Anim","",Color3.fromRGB(200,80,255),false,function(on)
    harderHitAnimEnabled=on; if on then startHarderHitAnim() else stopHarderHitAnim() end
end)
VisualSetters.HarderHitAnim = hhSetVis

local medSetVis,_,_ = mkToggle(pgCombat,O(),"Medusa Counter","",Color3.fromRGB(80,220,120),false,function(on)
...
mkInput(pgCombat,O(),"Normal Speed",tostring(NORMAL_SPEED),function(v,box)
    local n=tonumber(v); if n then NORMAL_SPEED=math.clamp(n,1,9999); box.Text=string.format("%.1f",NORMAL_SPEED) else box.Text=tostring(NORMAL_SPEED) end
end)
mkInput(pgCombat,O(),"Carry Speed",tostring(CARRY_SPEED),function(v,box)
    local n=tonumber(v); if n then CARRY_SPEED=math.clamp(n,1,9999); box.Text=string.format("%.1f",CARRY_SPEED) else box.Text=tostring(CARRY_SPEED) end
end)
mkInput(pgCombat,O(),"Lagger Speed",tostring(LAGGER_SPEED),function(v,box)
    local n=tonumber(v); if n then LAGGER_SPEED=math.clamp(n,1,9999); box.Text=string.format("%.1f",LAGGER_SPEED) else box.Text=tostring(LAGGER_SPEED) end
end)

-- ── MOVEMENT PAGE ────────────────────────────────────────────
ord=0
mkSec(pgMove,O(),"MOVEMENT")
local ijSetVis,_,_ = mkToggle(pgMove,O(),"Infinite Jump","InfiniteJump",SA,Enabled.InfiniteJump,function(s)
    Enabled.InfiniteJump=s; if s then startInfiniteJump() else stopInfiniteJump() end
end)
VisualSetters.InfiniteJump = ijSetVis

mkToggle(pgMove,O(),"Unwalk","",SM,Enabled.Unwalk,function(s)
    Enabled.Unwalk=s; if s then startUnwalk() else stopUnwalk() end
end)

local floatSetVis,_,_ = mkToggle(pgMove,O(),"Float","FloatToggle",Color3.fromRGB(120,180,255),false,function(on)
    floatEnabled=on; if on then startFloat() else stopFloat() end
end)

local dropSetVis,_,_ = mkToggle(pgMove,O(),"Drop Brainrot","DropBrainrot",SAMB,false,function(on)
    if on then task.spawn(runDropBrainrot) end
end)
setDropBrainrotVisual = dropSetVis

local tpDSetVis,_tpDB,_tpDC = mkToggle(pgMove,O(),"TP Down","TPDown",SMINT,false,function(on)
    if on then runTPDown() end
end)
VisualSetters.TPDown = tpDSetVis
VisualSetters.TPDownReset = function()
    if _tpDB  then tw(_tpDB, TIF,{BackgroundColor3=SOFF}) end
    if _tpDC  then tw(_tpDC, TIF,{Position=UDim2.fromOffset(3,3)}) end
    tpDSetVis(false)
end

local ncSetVis,_,_ = mkToggle(pgMove,O(),"No Cam Collision","",SM,false,function(on)
...
mkInput(pgMove,O(),"Float Height",tostring(floatHeight),function(v,box)
    local n=tonumber(v); if n then floatHeight=math.clamp(n,1,100); box.Text=tostring(floatHeight) else box.Text=tostring(floatHeight) end
end)

-- ── VISUAL PAGE ──────────────────────────────────────────────
ord=0
mkSec(pgVisual,O(),"VISUAL")
mkToggle(pgVisual,O(),"Anti Lag","",SM,Enabled.Optimizer,function(s)
    Enabled.Optimizer=s; if s then enableOptimizer() else disableOptimizer() end
end)
local ulSetVis,_,_ = mkToggle(pgVisual,O(),"Ultra Mode","",Color3.fromRGB(255,80,60),false,function(s)
    ultraModeEnabled=s; if s then enableUltraMode() else disableUltraMode() end
end)
VisualSetters.UltraMode = ulSetVis
mkToggle(pgVisual,O(),"Remove Accessories","",SM,Enabled.RemoveAccessories,function(s)
    Enabled.RemoveAccessories=s; if s then startRemoveAccessories() else stopRemoveAccessories() end
end)
mkSec(pgVisual,O(),"CONFIG")
do
    local container=Instance.new("Frame",pgVisual); container.Size=UDim2.new(1,-20,0,48)
    container.BackgroundColor3=SC; container.BorderSizePixel=0; container.LayoutOrder=O()
    Instance.new("UICorner",container).CornerRadius=UDim.new(0,12)
    local cSt3=Instance.new("UIStroke",container); cSt3.Color=SSD; cSt3.Thickness=1.2; cSt3.Transparency=0.4
    local lbl2=Instance.new("TextLabel",container); lbl2.Size=UDim2.new(1,-120,1,0)
    lbl2.Position=UDim2.new(0,16,0,0); lbl2.BackgroundTransparency=1; lbl2.Text="Reset Config"
    lbl2.TextColor3=ST; lbl2.Font=Enum.Font.GothamBold; lbl2.TextSize=12; lbl2.TextXAlignment=Enum.TextXAlignment.Left
    local resetBtn=Instance.new("TextButton",container); resetBtn.Size=UDim2.new(0,70,0,28)
    resetBtn.Position=UDim2.new(1,-80,0.5,-14); resetBtn.BackgroundColor3=SRED
    resetBtn.BorderSizePixel=0; resetBtn.Text="RESET"; resetBtn.TextColor3=SW
    ...
    resetBtn.MouseButton1Click:Connect(function()
        if not confirmed then
            confirmed=true; resetBtn.Text="SURE?"; resetBtn.BackgroundColor3=Color3.fromRGB(255,140,0)
            if confirmTimer then task.cancel(confirmTimer) end
            confirmTimer=task.delay(2,function() confirmed=false; resetBtn.Text="RESET"; resetBtn.BackgroundColor3=SRED end)
        else
            confirmed=false; if confirmTimer then task.cancel(confirmTimer) end
            resetBtn.Text="RESET"; resetBtn.BackgroundColor3=SRED
            if writefile then pcall(function() writefile("K7HubConfig.json","{}") end) end
            resetBtn.Text="DONE!"; resetBtn.BackgroundColor3=Color3.fromRGB(50,180,50)
            task.delay(1.5,function() resetBtn.Text="RESET"; resetBtn.BackgroundColor3=SRED end)
        end
    end)
    container.MouseEnter:Connect(function() tw(container,TIF,{BackgroundColor3=SCH}); cSt3.Transparency=0.1 end)
    container.MouseLeave:Connect(function() tw(container,TIF,{BackgroundColor3=SC});  cSt3.Transparency=0.4 end)
end

-- ── AUTO PAGE ────────────────────────────────────────────────
ord=0
mkSec(pgAuto,O(),"AUTO FEATURES")
local atpSetVis,_,_ = mkToggle(pgAuto,O(),"Auto TP","",SMINT,false,function(on)
    G_tpAutoEnabled=on; if on then task.spawn(function() refreshMyPlotSide() end) end
end)
VisualSetters.AutoTP = atpSetVis

local apSetVis,_,_ = mkToggle(pgAuto,O(),"Auto Play After TP","",SA,false,function(on)
    G_autoPlayAfterTP=on
end)
VisualSetters.APAfterTP = apSetVis

local cdSetVis,_,_ = mkToggle(pgAuto,O(),"Auto Play After Countdown","",SAMB,false,function(on)
...
-- Custom auto-steal toggle (with hold for settings)
local astCard=Instance.new("Frame",pgAuto); astCard.Size=UDim2.new(1,-20,0,60)
astCard.BackgroundColor3=SC; astCard.BorderSizePixel=0; astCard.LayoutOrder=O()
Instance.new("UICorner",astCard).CornerRadius=UDim.new(0,12)
local astCst=Instance.new("UIStroke",astCard); astCst.Color=SSD; astCst.Thickness=1.2; astCst.Transparency=0.4
local astBar=Instance.new("Frame",astCard); astBar.Size=UDim2.new(0,3,0,32)
astBar.Position=UDim2.new(0,0,0.5,-16); astBar.BackgroundColor3=SA; astBar.BorderSizePixel=0
Instance.new("UICorner",astBar).CornerRadius=UDim.new(1,0)
local astLbl=Instance.new("TextLabel",astCard); astLbl.Size=UDim2.new(1,-100,0,20)
astLbl.Position=UDim2.new(0,16,0,10); astLbl.BackgroundTransparency=1; astLbl.Text="Auto Steal"
astLbl.Font=Enum.Font.GothamBold; astLbl.TextSize=12; astLbl.TextColor3=ST; astLbl.TextXAlignment=Enum.TextXAlignment.Left
local astHint=Instance.new("TextLabel",astCard); astHint.Size=UDim2.new(1,-100,0,13)
astHint.Position=UDim2.new(0,16,0,30); astHint.BackgroundTransparency=1; astHint.Text="Right-click for settings"
astHint.Font=Enum.Font.Gotham; astHint.TextSize=9; astHint.TextColor3=SDM; astHint.TextXAlignment=Enum.TextXAlignment.Left
local astTrack=Instance.new("Frame",astCard); astTrack.Size=UDim2.new(0,44,0,24)
astTrack.Position=UDim2.new(1,-52,0.5,-12); astTrack.BackgroundColor3=Enabled.AutoSteal and SA or SOFF; astTrack.BorderSizePixel=0
Instance.new("UICorner",astTrack).CornerRadius=UDim.new(1,0)
local astTrkSt=Instance.new("UIStroke",astTrack); astTrkSt.Color=Enabled.AutoSteal and SA or SSD; astTrkSt.Thickness=1.2
local astKnob=Instance.new("Frame",astTrack); astKnob.Size=UDim2.new(0,18,0,18)
astKnob.Position=Enabled.AutoSteal and UDim2.fromOffset(23,3) or UDim2.fromOffset(3,3)
astKnob.BackgroundColor3=Enabled.AutoSteal and SW or SM; astKnob.BorderSizePixel=0
Instance.new("UICorner",astKnob).CornerRadius=UDim.new(1,0)
local astBtn=Instance.new("TextButton",astCard); astBtn.Size=UDim2.new(1,0,1,0)
astBtn.BackgroundTransparency=1; astBtn.Text=""; astBtn.ZIndex=5
...
astBtn.MouseButton1Click:Connect(function()
    ...
    astTrkSt.Color=Enabled.AutoSteal and SA or SSD
    astHint.TextColor3=Enabled.AutoSteal and SA or SDM
...
local function handleKeybind(kc)
    ...
    if kc==Keybinds.SpeedToggle then
        ...
        if speedSwBg and speedSwCircle then
            tw(speedSwBg,TIF,{BackgroundColor3=speedToggled and SCYAN or SOFF})
            tw(speedSwCircle,TIF,{Position=speedToggled and UDim2.fromOffset(23,3) or UDim2.fromOffset(3,3)})
        end
    ...
    if kc==Keybinds.LaggerToggle then
        ...
        if laggerSwBg and laggerSwCircle then
            tw(laggerSwBg,TIF,{BackgroundColor3=laggerToggled and Color3.fromRGB(255,80,60) or SOFF})
            tw(laggerSwCircle,TIF,{Position=laggerToggled and UDim2.fromOffset(23,3) or UDim2.fromOffset(3,3)})
        end
        if sideSetters and sideSetters.laggerSetter then sideSetters.laggerSetter(laggerToggled) end
    end
    if kc==Keybinds.AutoBat then
        ...
        if batSwBg and batSwCircle then
            tw(batSwBg,TIF,{BackgroundColor3=autoBatToggled and SA or SOFF})
            tw(batSwCircle,TIF,{Position=autoBatToggled and UDim2.fromOffset(23,3) or UDim2.fromOffset(3,3)})
        end
    end
    ...
    if kc==Keybinds.FloatToggle then
        ...
        if floatSetVis then floatSetVis(floatEnabled) end
...
UserInputService.InputBegan:Connect(function(input,gpe)
    ...
    elseif input.UserInputType==Enum.UserInputType.Gamepad1
        ...
        handleKeybind(mapped or input.KeyCode)
    end
end)
...
mobileButtonContainer.Size=UDim2.new(0,160,0,290); mobileButtonContainer.Position=UDim2.new(1,-170,0.5,-107)
mobileButtonContainer.BackgroundColor3=SB; mobileButtonContainer.BackgroundTransparency=0.3
...
do
    ...
    UserInputService.InputChanged:Connect(function(inp)
        if drag and (inp.UserInputType==Enum.UserInputType.MouseMovement or inp.UserInputType==Enum.UserInputType.Touch) then
            local d=inp.Position-ds; mobileButtonContainer.Position=UDim2.new(sp.X.Scale,sp.X.Offset+d.X,sp.Y.Scale,sp.Y.Offset+d.Y)
        end
    end)
end
local function createMobileButton(text,row,col,callback)
    local btn=Instance.new("TextButton",mobileButtonContainer)
    btn.Size=UDim2.new(0,70,0,65); btn.Position=UDim2.new(0,(col-1)*80+5,0,(row-1)*70+5)
    btn.BackgroundColor3=SC; btn.BorderSizePixel=0; btn.Text=""; btn.ZIndex=101
    Instance.new("UICorner",btn).CornerRadius=UDim.new(0,10)
    local btnSt=Instance.new("UIStroke",btn); btnSt.Color=SS; btnSt.Thickness=2; btnSt.Transparency=0.5
    local label=Instance.new("TextLabel",btn); label.Size=UDim2.new(1,0,0,25)
    label.Position=UDim2.new(0,0,0,8); label.BackgroundTransparency=1; label.Text=text
    label.TextColor3=SW; label.Font=Enum.Font.GothamBlack; label.TextSize=11; label.ZIndex=102
    local circle=Instance.new("Frame",btn); circle.Size=UDim2.new(0,14,0,14)
    circle.Position=UDim2.new(0.5,-7,1,-18); circle.BackgroundColor3=SB; circle.BorderSizePixel=0; circle.ZIndex=102
    Instance.new("UICorner",circle).CornerRadius=UDim.new(1,0)
    local cirSt=Instance.new("UIStroke",circle); cirSt.Color=SW; cirSt.Thickness=2
    local cirFill=Instance.new("Frame",circle); cirFill.Size=UDim2.new(0,0,0,0)
    cirFill.Position=UDim2.new(0.5,0,0.5,0); cirFill.AnchorPoint=Vector2.new(0.5,0.5)
    cirFill.BackgroundColor3=SW; cirFill.BorderSizePixel=0; cirFill.ZIndex=103
    Instance.new("UICorner",cirFill).CornerRadius=UDim.new(1,0)
    local isActive=false
    local function setVis(state)
        isActive=state
        TweenService:Create(cirFill,TweenInfo.new(0.2,Enum.EasingStyle.Back),{Size=isActive and UDim2.new(1,-4,1,-4) or UDim2.new(0,0,0,0)}):Play()
        TweenService:Create(btnSt,TweenInfo.new(0.2),{Transparency=isActive and 0 or 0.5}):Play()
    end
...
local batSetter = createMobileButton("BAT",1,1,function(on)
    autoBatToggled=on
    if batSwBg and batSwCircle then
        tw(batSwBg,TIF,{BackgroundColor3=on and SA or SOFF})
        tw(batSwCircle,TIF,{Position=on and UDim2.fromOffset(23,3) or UDim2.fromOffset(3,3)})
    end
end)
local carrySetter = createMobileButton("CARRY",1,2,function(on)
    ...
    speedToggled=on
    if speedSwBg and speedSwCircle then
        tw(speedSwBg,TIF,{BackgroundColor3=on and SCYAN or SOFF})
        tw(speedSwCircle,TIF,{Position=on and UDim2.fromOffset(23,3) or UDim2.fromOffset(3,3)})
    end
end)
local floatMobSetter = createMobileButton("FLOAT",2,2,function(on)
    floatEnabled=on; if on then startFloat() else stopFloat() end
end)
local dropMobV = createMobileButton("DROP",4,1,function(on)
    if on then if setDropBrainrotVisual then setDropBrainrotVisual(true) end; task.spawn(runDropBrainrot) end
end)
dropMobileSetter = dropMobV
fullAutoLeftSetter = createMobileButton("FULL\\nAUTO L",3,1,function(on)
    fullAutoPlayLeftEnabled=on; if on then startFullAutoLeft() else stopFullAutoLeft() end
end)
fullAutoRightSetter = createMobileButton("FULL\\nAUTO R",3,2,function(on)
    fullAutoPlayRightEnabled=on; if on then startFullAutoRight() else stopFullAutoRight() end
end)
local laggerSetter = createMobileButton("LAG",2,1,function(on)
    ...
    laggerToggled=on
    if laggerSwBg and laggerSwCircle then
        tw(laggerSwBg,TIF,{BackgroundColor3=on and Color3.fromRGB(255,80,60) or SOFF})
        tw(laggerSwCircle,TIF,{Position=on and UDim2.fromOffset(23,3) or UDim2.fromOffset(3,3)})
    end
end)
...
local function setupChar(char)
    ...
    if head then
        ...
        speedLbl.BackgroundTransparency=1; speedLbl.TextColor3=SA
...
task.spawn(function()
    task.wait(1)
    batSetter(autoBatToggled); carrySetter(speedToggled)
    laggerSetter(laggerToggled); floatMobSetter(floatEnabled)
end)
task.spawn(function()
    task.wait(2)
...
-- panel slide-in after loader
Shadow.Position = UDim2.new(0.5,-146,0.5,-280)
main.BackgroundTransparency=1; Shadow.BackgroundTransparency=1
...
task.delay(2.3,function()
    TweenService:Create(Shadow,TweenInfo.new(0.5,Enum.EasingStyle.Back,Enum.EasingDirection.Out),
        {Position=UDim2.new(0.5,-146,0.5,-240),BackgroundTransparency=0.82}):Play()
    TweenService:Create(main,TweenInfo.new(0.5,Enum.EasingStyle.Back,Enum.EasingDirection.Out),
        {BackgroundTransparency=0}):Play()
end)

print("[SWEETY K7 HUB] Loaded - Void Blue Design")
end -- _buildGUI
''')
...
local function apSaveConfig()
    pcall(function()
        ...
        writefile(SAVE_FILE, HttpService2:JSONEncode({
            speed=AP_NORMAL_SPEED,
            autoLeft=AP_Keybinds.AutoLeft.Name, autoRight=AP_Keybinds.AutoRight.Name,
            uiToggle=AP_Keybinds.UIToggle.Name,
            fullAutoLeft=AP_Keybinds.FullAutoLeft.Name,
...
local function apLoadConfig()
    pcall(function()
        ...
        local ok,cfg=pcall(function() return HttpService2:JSONDecode(readfile(SAVE_FILE)) end)
        if not ok or not cfg then return end
...
local AP_LeftOn=false; local AP_RightOn=false
...
local function apGetKeyDisplayName(kc)
    local name=kc.Name
    local gpMap={ButtonA="A",ButtonB="B",ButtonX="X",ButtonY="Y",ButtonL1="L1",ButtonL2="L2",ButtonR1="R1",ButtonR2="R2",ButtonStart="Start",DPadLeft="D\\226\\134\\144",DPadRight="D\\226\\134\\146",DPadUp="D\\226\\134\\145",DPadDown="D\\226\\134\\147"}
...
RunService2.RenderStepped:Connect(function()
    ...
    local h2=char:FindFirstChild("HumanoidRootPart"); local hum=char:FindFirstChildOfClass("Humanoid")
    if not h2 or not hum then return end
    local md=hum.MoveDirection
    if md.Magnitude>0 then
        h2.AssemblyLinearVelocity=Vector3.new(md.X*AP_NORMAL_SPEED,h2.AssemblyLinearVelocity.Y,md.Z*AP_NORMAL_SPEED)
    end
end)
local function AP_FaceSouth()
    local c=LocalPlayer2.Character; if not c then return end
...
local function AP_StartLeft()
    ...
    AP_LeftConn=RunService2.Heartbeat:Connect(function()
        ...
        if not rp or not hum then return end; local spd=AP_NORMAL_SPEED
        if AP_LeftPhase==1 then
            local tgt=Vector3.new(AP_L1.X,rp.Position.Y,AP_L1.Z)
            if (tgt-rp.Position).Magnitude<1 then
                AP_LeftPhase=2; local d=AP_L2-rp.Position; local mv=Vector3.new(d.X,0,d.Z).Unit
        ...
        elseif AP_LeftPhase==2 then
            ...
            if (tgt-rp.Position).Magnitude<1 then
                ...
                AP_LeftPhase=1; if AP_SetLeftVisual then AP_SetLeftVisual(false) end; AP_FaceSouth(); return
            end
...
local function AP_StopLeft()
    if AP_LeftConn then AP_LeftConn:Disconnect(); AP_LeftConn=nil end; AP_LeftPhase=1
    local c=LocalPlayer2.Character; if c then local hum=c:FindFirstChildOfClass("Humanoid"); if hum then hum:Move(Vector3.zero,false) end end
end
local function AP_StartRight()
    ...
    AP_RightConn=RunService2.Heartbeat:Connect(function()
        ...
        if AP_RightPhase==1 then
            ...
            if (tgt-rp.Position).Magnitude<1 then
                AP_RightPhase=2; local d=AP_R2-rp.Position; local mv=Vector3.new(d.X,0,d.Z).Unit
                hum:Move(mv,false); rp.AssemblyLinearVelocity=Vector3.new(mv.X*spd,rp.AssemblyLinearVelocity.Y,mv.Z*spd); return
            end
        ...
        elseif AP_RightPhase==2 then
            ...
            if (tgt-rp.Position).Magnitude<1 then
                ...
                AP_RightPhase=1; if AP_SetRightVisual then AP_SetRightVisual(false) end; AP_FaceNorth(); return
            end
...
local function AP_StopRight()
    if AP_RightConn then AP_RightConn:Disconnect(); AP_RightConn=nil end; AP_RightPhase=1
    local c=LocalPlayer2.Character; if c then local hum=c:FindFirstChildOfClass("Humanoid"); if hum then hum:Move(Vector3.zero,false) end end
end

-- VOID BLUE PALETTE for AP panel
local AP_BG   = Color3.fromRGB(8,10,18)
local AP_CARD = Color3.fromRGB(12,15,28)
local AP_CARDH= Color3.fromRGB(16,20,38)
local AP_ACC  = Color3.fromRGB(60,140,255)
local AP_ACCD = Color3.fromRGB(20,50,120)
local AP_RED  = Color3.fromRGB(220,30,30)
local AP_ST   = Color3.fromRGB(40,80,180)
local AP_SSD  = Color3.fromRGB(20,35,80)
local AP_OFF  = Color3.fromRGB(18,22,45)
local AP_TEXT = Color3.fromRGB(220,235,255)
local AP_MUT  = Color3.fromRGB(100,120,170)
local AP_DIM  = Color3.fromRGB(50,65,110)
local AP_WHITE= Color3.fromRGB(255,255,255)
local AP_MINT = Color3.fromRGB(60,255,160)
...
local apGui2=Instance.new("ScreenGui")
apGui2.Name="AutoPlayGUI2"; apGui2.ResetOnSpawn=false; apGui2.DisplayOrder=10
apGui2.Parent=LocalPlayer2:WaitForChild("PlayerGui")

apMain=Instance.new("Frame"); apMain.Name="Main"
apMain.Size=UDim2.new(0,300,0,400); apMain.Position=UDim2.new(0.5,-150,0.5,-200)
apMain.BackgroundColor3=AP_BG; apMain.BorderSizePixel=0; apMain.Active=true; apMain.ClipsDescendants=true
apMain.Parent=apGui2
Instance.new("UICorner",apMain).CornerRadius=UDim.new(0,16)
local apStroke2=Instance.new("UIStroke",apMain); apStroke2.Color=AP_ST; apStroke2.Thickness=1.5
...
do
    ...
    UserInputService2.InputChanged:Connect(function(inp)
        if drag and (inp.UserInputType==Enum.UserInputType.MouseMovement or inp.UserInputType==Enum.UserInputType.Touch) then
            local d=inp.Position-ds; apMain.Position=UDim2.new(sp.X.Scale,sp.X.Offset+d.X,sp.Y.Scale,sp.Y.Offset+d.Y)
        end
    end)
end

-- header
local apHdr=Instance.new("Frame",apMain); apHdr.Size=UDim2.new(1,0,0,54)
apHdr.BackgroundColor3=Color3.fromRGB(6,8,14); apHdr.BorderSizePixel=0; apHdr.ZIndex=4
Instance.new("UICorner",apHdr).CornerRadius=UDim.new(0,16)
local apHFix=Instance.new("Frame",apHdr); apHFix.Size=UDim2.new(1,0,0,16)
apHFix.Position=UDim2.new(0,0,1,-16); apHFix.BackgroundColor3=Color3.fromRGB(6,8,14); apHFix.BorderSizePixel=0; apHFix.ZIndex=3
local apIco=Instance.new("TextLabel",apHdr); apIco.Size=UDim2.new(0,32,0,32)
apIco.Position=UDim2.new(0,12,0.5,-16); apIco.BackgroundColor3=AP_ACCD; apIco.BorderSizePixel=0
apIco.Text="AP"; apIco.TextSize=13; apIco.Font=Enum.Font.GothamBlack; apIco.TextColor3=AP_ACC
Instance.new("UICorner",apIco).CornerRadius=UDim.new(0,8)
local apTitle=Instance.new("TextLabel",apHdr); apTitle.Size=UDim2.new(1,-100,0,20)
apTitle.Position=UDim2.new(0,52,0,8); apTitle.BackgroundTransparency=1; apTitle.Text="AUTO PLAY"
apTitle.TextColor3=AP_TEXT; apTitle.Font=Enum.Font.GothamBlack; apTitle.TextSize=14; apTitle.TextXAlignment=Enum.TextXAlignment.Left; apTitle.ZIndex=6
local apSub=Instance.new("TextLabel",apHdr); apSub.Size=UDim2.new(1,-100,0,13)
apSub.Position=UDim2.new(0,52,0,29); apSub.BackgroundTransparency=1; apSub.Text="discord.gg/k7hub"
apSub.TextColor3=AP_MUT; apSub.Font=Enum.Font.GothamMedium; apSub.TextSize=9; apSub.TextXAlignment=Enum.TextXAlignment.Left; apSub.ZIndex=5
local apClose=Instance.new("TextButton",apHdr); apClose.Size=UDim2.new(0,28,0,28)
apClose.Position=UDim2.new(1,-34,0.5,-14); apClose.BackgroundColor3=AP_RED; apClose.BorderSizePixel=0
apClose.Text="x"; apClose.TextColor3=AP_WHITE; apClose.Font=Enum.Font.GothamBold; apClose.TextSize=18; apClose.ZIndex=8
Instance.new("UICorner",apClose).CornerRadius=UDim.new(0,6)
local apHLine=Instance.new("Frame",apHdr); apHLine.Size=UDim2.new(0.9,0,0,1)
apHLine.Position=UDim2.new(0.05,0,1,-1); apHLine.BackgroundColor3=AP_SSD; apHLine.BorderSizePixel=0

local apReopen=Instance.new("TextButton",apGui2); apReopen.Size=UDim2.new(0,45,0,45)
apReopen.Position=UDim2.new(0.5,-22.5,0.5,-22.5); apReopen.BackgroundColor3=AP_ACC; apReopen.BorderSizePixel=0
apReopen.Text="AP"; apReopen.TextColor3=AP_WHITE; apReopen.Font=Enum.Font.GothamBlack; apReopen.TextSize=14; apReopen.ZIndex=20; apReopen.Visible=false
...
apClose.MouseEnter:Connect(function() apTw(apClose,AP_TIF,{BackgroundColor3=Color3.fromRGB(255,60,80)}) end)
apClose.MouseLeave:Connect(function() apTw(apClose,AP_TIF,{BackgroundColor3=AP_RED}) end)

-- scroll
local apScroll=Instance.new("ScrollingFrame",apMain); apScroll.Size=UDim2.new(1,-20,1,-64)
apScroll.Position=UDim2.new(0,10,0,58); apScroll.BackgroundTransparency=1; apScroll.BorderSizePixel=0
apScroll.ScrollBarThickness=3; apScroll.ScrollBarImageColor3=AP_ACC
apScroll.AutomaticCanvasSize=Enum.AutomaticSize.Y; apScroll.CanvasSize=UDim2.new(0,0,0,0)
local apVS=Instance.new("Frame",apScroll); apVS.Size=UDim2.new(1,0,0,0)
apVS.AutomaticSize=Enum.AutomaticSize.Y; apVS.BackgroundTransparency=1
...
local function apRow(keybindName,labelTxt,onToggle)
    local container=Instance.new("Frame",apVS); container.Size=UDim2.new(1,0,0,60)
    container.BackgroundColor3=AP_CARD; container.BorderSizePixel=0; container.LayoutOrder=APO()
    Instance.new("UICorner",container).CornerRadius=UDim.new(0,12)
    local cSt2=Instance.new("UIStroke",container); cSt2.Color=AP_SSD; cSt2.Thickness=1.2; cSt2.Transparency=0.4
    local bar=Instance.new("Frame",container); bar.Size=UDim2.new(0,3,0,32)
    bar.Position=UDim2.new(0,0,0.5,-16); bar.BackgroundColor3=AP_ACC; bar.BorderSizePixel=0
    Instance.new("UICorner",bar).CornerRadius=UDim.new(1,0)
    local kbBg=Instance.new("Frame",container); kbBg.Size=UDim2.new(0,36,0,36)
    kbBg.Position=UDim2.new(0,10,0.5,-18); kbBg.BackgroundColor3=AP_ACCD; kbBg.BackgroundTransparency=0.3; kbBg.BorderSizePixel=0; kbBg.ZIndex=5
    Instance.new("UICorner",kbBg).CornerRadius=UDim.new(0,9)
    local kbSt2=Instance.new("UIStroke",kbBg); kbSt2.Color=AP_ACC; kbSt2.Thickness=1.5; kbSt2.Transparency=0.4
    local kbTxt=Instance.new("TextLabel",kbBg); kbTxt.Size=UDim2.new(1,0,1,0); kbTxt.BackgroundTransparency=1
    kbTxt.Text=apGetKeyDisplayName(AP_Keybinds[keybindName]); kbTxt.TextColor3=AP_WHITE
    kbTxt.Font=Enum.Font.GothamBold; kbTxt.TextSize=10; kbTxt.ZIndex=6
    local nameLbl=Instance.new("TextLabel",container); nameLbl.Size=UDim2.new(1,-148,0,20)
    nameLbl.Position=UDim2.new(0,54,0,10); nameLbl.BackgroundTransparency=1; nameLbl.Text=labelTxt
    nameLbl.TextColor3=AP_TEXT; nameLbl.Font=Enum.Font.GothamBold; nameLbl.TextSize=12; nameLbl.TextXAlignment=Enum.TextXAlignment.Left; nameLbl.ZIndex=5
    local hintLbl=Instance.new("TextLabel",container); hintLbl.Size=UDim2.new(1,-148,0,13)
    hintLbl.Position=UDim2.new(0,54,0,30); hintLbl.BackgroundTransparency=1; hintLbl.Text="Hold to rebind"
    hintLbl.Font=Enum.Font.Gotham; hintLbl.TextSize=9; hintLbl.TextColor3=AP_DIM; hintLbl.TextXAlignment=Enum.TextXAlignment.Left
    local track=Instance.new("Frame",container); track.Size=UDim2.new(0,44,0,24)
    track.Position=UDim2.new(1,-52,0.5,-12); track.BackgroundColor3=AP_OFF; track.BorderSizePixel=0
    Instance.new("UICorner",track).CornerRadius=UDim.new(1,0)
    local trkSt2=Instance.new("UIStroke",track); trkSt2.Color=AP_SSD; trkSt2.Thickness=1.2
    local knob=Instance.new("Frame",track); knob.Size=UDim2.new(0,18,0,18)
    knob.Position=UDim2.fromOffset(3,3); knob.BackgroundColor3=AP_MUT; knob.BorderSizePixel=0
    Instance.new("UICorner",knob).CornerRadius=UDim.new(1,0)
    local isOn=false
    local function setVis(state)
        isOn=state
        apTw(track,AP_TIF,{BackgroundColor3=isOn and AP_ACC or AP_OFF})
        apTw(knob,AP_TIF,{BackgroundColor3=isOn and AP_WHITE or AP_MUT, Position=isOn and UDim2.fromOffset(23,3) or UDim2.fromOffset(3,3)})
        trkSt2.Color=isOn and AP_ACC or AP_SSD
        hintLbl.TextColor3=isOn and AP_ACC or AP_DIM
        apTw(bar,AP_TIF,{BackgroundColor3=isOn and AP_ACC or AP_SSD})
    end
    local isHolding=false; local holdStart=0
    local function startKBChange()
        AP_WFK=keybindName; kbTxt.Text="..."
        apTw(kbBg,AP_TIF,{BackgroundColor3=Color3.fromRGB(100,70,0),BackgroundTransparency=0})
        local conn; conn=UserInputService2.InputBegan:Connect(function(inp,_)
            ...
            local kc=inp.KeyCode; if kc==Enum.KeyCode.Unknown then return end
            if isKB and kc==Enum.KeyCode.Escape then
                kbTxt.Text=apGetKeyDisplayName(AP_Keybinds[keybindName])
                apTw(kbBg,AP_TIF,{BackgroundColor3=AP_ACCD,BackgroundTransparency=0.3})
                AP_WFK=nil; conn:Disconnect(); return
            end
            ...
            apTw(kbBg,AP_TIF,{BackgroundColor3=AP_ACCD,BackgroundTransparency=0.3})
            AP_WFK=nil; conn:Disconnect(); apSaveConfig()
        end)
    end
    local clk=Instance.new("TextButton",container); clk.Size=UDim2.new(1,0,1,0)
    clk.BackgroundTransparency=1; clk.Text=""; clk.ZIndex=7
...
local function apInput(lbl,defaultTxt,onDone)
    local wrap=Instance.new("Frame",apVS); wrap.Size=UDim2.new(1,0,0,48)
    wrap.BackgroundColor3=AP_CARD; wrap.BorderSizePixel=0; wrap.LayoutOrder=APO()
    ...
    local barI=Instance.new("Frame",wrap); barI.Size=UDim2.new(0,3,0,28)
    barI.Position=UDim2.new(0,0,0.5,-14); barI.BackgroundColor3=AP_ACC; barI.BorderSizePixel=0
    Instance.new("UICorner",barI).CornerRadius=UDim.new(1,0)
    local lbTxt=Instance.new("TextLabel",wrap); lbTxt.Size=UDim2.new(0.5,0,1,0)
    lbTxt.Position=UDim2.new(0,16,0,0); lbTxt.BackgroundTransparency=1; lbTxt.Text=lbl
    lbTxt.TextColor3=AP_TEXT; lbTxt.Font=Enum.Font.GothamBold; lbTxt.TextSize=12; lbTxt.TextXAlignment=Enum.TextXAlignment.Left
    local box=Instance.new("TextBox",wrap); box.Size=UDim2.new(0,80,0,28)
    box.Position=UDim2.new(1,-90,0.5,-14); box.BackgroundColor3=Color3.fromRGB(6,8,14); box.BorderSizePixel=0
    box.Text=defaultTxt; box.TextColor3=AP_TEXT; box.Font=Enum.Font.GothamBold; box.TextSize=11; box.ClearTextOnFocus=false
    Instance.new("UICorner",box).CornerRadius=UDim.new(0,8)
    local bSt=Instance.new("UIStroke",box); bSt.Color=AP_ACC; bSt.Thickness=1; bSt.Transparency=0.5
    box.Focused:Connect(function() apTw(bSt,AP_TIF,{Transparency=0}) end)
    box.FocusLost:Connect(function() apTw(bSt,AP_TIF,{Transparency=0.5}); if onDone then onDone(box.Text,box) end end)
    wrap.MouseEnter:Connect(function() apTw(wrap,AP_TIF,{BackgroundColor3=AP_CARDH}); wSt.Transparency=0.1 end)
    wrap.MouseLeave:Connect(function() apTw(wrap,AP_TIF,{BackgroundColor3=AP_CARD}); wSt.Transparency=0.4 end)
end

apSec("FULL AUTO PLAY")
local falVis,_ = apRow("FullAutoLeft","Full Auto Left",function(on)
    fullAutoPlayLeftEnabled=on; if on then startFullAutoLeft() else stopFullAutoLeft() end
end)
fullAutoLeftSetter=falVis
local farVis,_ = apRow("FullAutoRight","Full Auto Right",function(on)
    fullAutoPlayRightEnabled=on; if on then startFullAutoRight() else stopFullAutoRight() end
end)
fullAutoRightSetter=farVis
apSec("AUTO PLAY")
local leftVis,_ = apRow("AutoLeft","Auto Left",function(on)
    AP_LeftOn=on; if on then AP_StartLeft() else AP_StopLeft() end
end)
AP_SetLeftVisual=leftVis
local rightVis,_ = apRow("AutoRight","Auto Right",function(on)
    AP_RightOn=on; if on then AP_StartRight() else AP_StopRight() end
end)
AP_SetRightVisual=rightVis
apSec("SETTINGS")
apInput("Speed",tostring(AP_NORMAL_SPEED),function(v,box)
    local n=tonumber(v); if n then AP_NORMAL_SPEED=math.clamp(n,1,9999); box.Text=tostring(AP_NORMAL_SPEED); apSaveConfig() else box.Text=tostring(AP_NORMAL_SPEED) end
end)
...
UserInputService2.InputBegan:Connect(function(input,gpe)
    if gpe then return end
...
print("[SWEETY K7 HUB] Auto Play loaded - Z=AutoLeft | C=AutoRight | P=Toggle")
end)()
''')
...
-- ============================================================
_buildGUI()
print("[SWEETY K7 HUB] All systems loaded!")
''')

full = "\n".join(lines)

with open('/mnt/user-data/outputs/sweety_k7_hub.lua', 'w', encoding='utf-8') as f:
    f.write(full)

print("Done! Lines:", len(full.splitlines()), "Bytes:", len(full.encode('utf-8')))
PYEOF
